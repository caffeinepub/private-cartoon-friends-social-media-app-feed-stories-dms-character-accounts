...
import type { CharacterProfileView, MessageView } from "../backend";
import {
...
export default function MessageThread({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: intentional scroll effect
  useEffect(() => {
    ...
    if (conversation && characters) {
      ...
      if (firstCharId) {
        const char = characters.find(
          (c: CharacterProfileView) => c.id === firstCharId,
        );
  ...
  const headerChar =
    participantIds.length > 0 ? getCharacter(participantIds[0]) : undefined;
  const headerTitle =
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {!conversation.messages || conversation.messages.length === 0 ? (
          ...
          conversation.messages.map((message: MessageView) => {
            ...
            const senderChar = isUser
              ? undefined
              : getCharacter(message.sender);
